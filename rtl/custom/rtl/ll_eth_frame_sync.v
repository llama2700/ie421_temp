/*
 * Ultra-low-latency 10GBASE-R block alignment / bitslip controller.
 *
 * Simplified from Corundum's eth_phy_10g_rx_frame_sync.v.
 * Same algorithm: count valid vs invalid sync headers over 64-block windows,
 * trigger bitslip on excessive invalid headers, assert rx_block_lock after
 * 64 consecutive valid headers with zero invalid.
 */
...
localparam BITSLIP_MAX_CYCLES   = (BITSLIP_HIGH_CYCLES > BITSLIP_LOW_CYCLES)
                                  ? BITSLIP_HIGH_CYCLES : BITSLIP_LOW_CYCLES;
localparam BITSLIP_COUNT_WIDTH  = (BITSLIP_MAX_CYCLES > 1) ? $clog2(BITSLIP_MAX_CYCLES) : 1;
