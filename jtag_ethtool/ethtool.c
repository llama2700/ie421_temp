...
static int do_geeprom(struct cmd_context *ctx)
{
	...
	/* 原代码：err = send_ioctl(ctx, eeprom); */
    /* 修改后：通过 JTAG 逐字节填充缓冲区 */
    printf("Starting JTAG read for %d bytes...\n", eeprom->len);
    for (int i = 0; i < eeprom->len; i++) {
        int val = jtag_qsfp_read_byte(eeprom->offset + i);
        eeprom->data[i] = (__u8)val;
    }
    err = 0; // 标记成功
	/*End of modfication*/
	if (err < 0) {
